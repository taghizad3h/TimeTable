...
import al.taghizadeh.csp.CspSolver;
import al.taghizadeh.csp.Variable;
import al.taghizadeh.csp.inference.InferenceLog;
...
public abstract class AbstractBacktrackingSolver<VAR extends Variable, VAL> extends CspSolver<VAR, VAL> {
    ...
    Logger logger = Logger.getLogger(AbstractBacktrackingSolver.class);

    ToDouble<VAR, VAL> distancer;
    ...
    private Assignment<VAR, VAL> backtrack(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment) {
        logger.info("finding with backtrack");
        distancer = new ToDouble<>();
            ...
            for (VAL value : orderDomainValues(csp, assignment, var)) {
                ...
                if (assignment.isConsistent(csp.getConstraints(var))) {
                    ...
                    if (!log.inconsistencyFound()) {
                        ...
                        if (result != null) {
                            if(result.isComplete(result.getVariables()))
                            logger.info("found ans answer with distance of " + distancer.applyAsDouble(result));
                            break;
                        }
                    }
                    log.undo(csp);
